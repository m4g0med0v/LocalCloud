...
export function UserDetailSheet({ user, onClose }: Props) {
  ...
  const approve = useMutation({
    ...
    onSuccess: () => { invalidate(); toast.success("Одобрен"); },
  ...
  const unblock = useMutation({
    ...
    onSuccess: () => { invalidate(); toast.success("Разблокирован"); },
          ...
          <div className="p-5">
            ...
            {quotaLoading ? (
              ...
              <div className="flex flex-col gap-3">
                ...
                <QuotaStat
                  label="Файлы"
                  used={quota.files_used}
                  limit={quota.files_limit}
                />
                <QuotaStat
                  ...
                  <div className="mt-1 flex flex-col gap-2.5 rounded-lg border p-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex flex-col gap-1">
                        <label className="text-xs text-muted-foreground">Хранилище (ГБ)</label>
                        <Input
                          type="number"
                          min="0"
                          step="1"
                          placeholder={String(
                            Math.round(quota.storage_limit_bytes / 1024 ** 3),
                          )}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-xs text-muted-foreground">Макс. файл (МБ)</label>
                        <Input
                          type="number"
                          min="0"
                          step="1"
                          placeholder={String(
                            Math.round(quota.max_file_size_bytes / 1024 ** 2),
                          )}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-xs text-muted-foreground">Лимит файлов</label>
                        <Input
                          type="number"
                          min="0"
                          step="1"
                          placeholder={
                            quota.files_limit == null ? "∞" : String(quota.files_limit)
                          }
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-xs text-muted-foreground">Лимит ссылок</label>
                        <Input
                          type="number"
                          min="0"
                          step="1"
