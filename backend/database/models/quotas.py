...
class UserQuota(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __tablename__ = "user_quotas"  # pyright: ignore[reportAssignmentType]
    ...
    def usage_percent(self) -> float:
        ...
        Если ``storage_limit_bytes`` равен нулю, возвращает:
            - 100.0, если занято больше 0 байт;
            - 0.0, если ничего не занято.
    ...
    def is_storage_full(self) -> bool:
        """Возвращает True, если использованное хранилище достигло лимита."""
    ...
    def is_active_upload_sessions_limit_reached(self) -> bool:
        """Возвращает True, если достигнут лимит активных upload-сессий."""
    ...
    def can_store_file_size(self, file_size_bytes: int) -> bool:
        ...
        Returns:
            True, если файл можно сохранить.
        """
    ...
    def can_create_file(self, file_size_bytes: int) -> bool:
        """Возвращает True, если пользователь может создать файл."""

        return self.can_store_file_size(file_size_bytes)

    def can_create_public_link(self) -> bool:
        """Возвращает True, если пользователь может создать публичную ссылку."""

        return not self.is_public_links_limit_reached

    def can_create_upload_session(self) -> bool:
        """Возвращает True, если пользователь может создать upload-сессию."""
    ...
    def update_limits(
        ...
        ``None`` для ``storage_limit_bytes`` и ``max_file_size_bytes`` означает,
        что соответствующий лимит не изменяется.
