...
class RefreshToken(Base, UUIDPrimaryKeyMixin, CreatedAtMixin):
    ...
    def is_revoked(self) -> bool:
        """Возвращает True, если токен был явно отозван."""

        return self.revoked_at is not None or self.status == SessionStatus.REVOKED

    @property
    def is_replaced(self) -> bool:
        """Возвращает True, если токен был заменён другим refresh-токеном."""

        return self.replaced_by_token_id is not None

    @property
    def is_expired(self) -> bool:
        """Возвращает True, если токен помечен как истёкший.

        Для проверки относительно конкретного времени используйте
        ``is_expired_at()``.
        """
    ...
    def replace_with(
        ...
        Используется при ротации refresh-токенов.
