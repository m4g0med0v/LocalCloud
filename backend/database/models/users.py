...
class User(Base, TimestampMixin):
    ...
    def is_active(self) -> bool:
        """Возвращает True, если пользователь может использовать систему."""

        return self.status == UserStatus.ACTIVE

    @property
    def is_blocked(self) -> bool:
        """Возвращает True, если учётная запись пользователя заблокирована."""

        return self.status == UserStatus.BLOCKED

    @property
    def is_pending(self) -> bool:
        """Возвращает True, если пользователь ожидает одобрения."""

        return self.status == UserStatus.PENDING

    @property
    def is_rejected(self) -> bool:
        """Возвращает True, если регистрация пользователя отклонена."""

        return self.status == UserStatus.REJECTED

    @property
    def is_deleted(self) -> bool:
        """Возвращает True, если учётная запись логически удалена."""

        return self.status == UserStatus.DELETED

    @property
    def can_login(self) -> bool:
        """Возвращает True, если пользователь имеет право войти в систему."""
    ...
    def is_regular_user(self) -> bool:
        """Возвращает True, если у пользователя есть базовая роль пользователя."""
