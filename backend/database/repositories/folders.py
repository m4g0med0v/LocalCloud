...
class FolderRepository(BaseRepository[Folder]):
    ...
    Операции над деревом файловой системы делегируются в `FileSystemNodeRepository`.
    ...
    async def create_folder(
        ...
        *,
        ...
        Raises:
            InvalidQueryError: Если имя, родительский узел или цветовая метка некорректны.
            EntityNotFoundError: Если владелец или родительский узел не найден.
            DuplicateEntityError: Если в родительской папке уже есть папка или узел с таким именем.
    ...
    async def create_for_existing_node(
        ...
        *,
        ...
        Args:
            ...
            flush: Выполнить ли `flush` после создания.
            refresh: Выполнить ли `refresh` после создания.
            check_duplicate: Проверять ли наличие уже существующей записи `Folder`.
        ...
        Raises:
            ...
            InvalidQueryError: Если узел не является папкой или цветовая метка некорректна.
    ...
    async def list_user_folders(
        ...
        *,
        ...
        Если `parent_id` передан, возвращаются папки внутри указанной родительской
        папки. Если `parent_id=None`, возвращаются корневые папки пользователя.
    ...
    async def rename_folder(
        ...
        *,
        ...
        Операция делегируется в `FileSystemNodeRepository`, поскольку имя, путь
        и конфликт имён относятся к узлу файловой системы.
        ...
        Raises:
            ...
            DuplicateEntityError: Если в родительской папке уже есть узел с таким именем.
        """
    ...
    async def move_folder(
        ...
        *,
        ...
        Операция делегируется в `FileSystemNodeRepository`, поскольку перемещение
        требует обновления `parent_id`, `path`, `depth` и путей потомков.

        Args:
            node_id: Идентификатор узла папки.
            new_parent_id: Идентификатор новой родительской папки или `None` для корня.
        ...
        Raises:
            EntityNotFoundError: Если папка, узел или родительская папка не найдены.
            InvalidQueryError: Если узел не является папкой или перемещение некорректно.
            DuplicateEntityError: Если в целевой папке уже есть узел с таким именем.
        """
    ...
    async def search_folders(
        ...
        *,
        ...
        Поиск выполняется по имени узла, материализованному пути и описанию папки.
        Дополнительно можно фильтровать по родительской папке, цвету и признаку удаления.
        ...
        Raises:
            InvalidQueryError: Если параметры пагинации, сортировки или цветовая метка некорректны.
        """
