...
class UploadSessionsRepository(BaseRepository[UploadSession]):
    ...
    Репозиторий не выполняет `commit` или `rollback`. Управление транзакциями
    должно находиться на уровне сервисного слоя или Unit of Work.
    ...
    async def get_by_upload_id(
        ...
        Args:
            upload_id: Внешний идентификатор multipart-загрузки в объектном хранилище.
    ...
    async def is_upload_id_exists(
        ...
        Returns:
            `True`, если сессия с таким `upload_id` существует, иначе `False`.
        """
    ...
    async def list_parent_sessions(
        ...
        *,
        ...
        """Возвращает сессии загрузки, связанные с родительским узлом файловой системы.

        Обычно используется для получения загрузок внутри конкретной папки назначения.
        Нельзя одновременно передавать `status` и `statuses`.
        ...
        Raises:
            InvalidQueryError: Если параметры фильтрации или пагинации некорректны.
        """
    ...
    async def create_session(
        ...
        *,
        ...
        Перед созданием нормализует строковые значения, валидирует параметры сессии,
        при необходимости проверяет существование пользователя, родительского узла
        файловой системы и уникальность `upload_id`.
    ...
    async def mark_completed(
        ...
        *,
        ...
        Если `require_all_parts=True`, перед завершением проверяет, что загружены
        все части файла.

        Args:
            upload_session_id: Идентификатор сессии загрузки.
            completed_at: Дата завершения. Если не передана, используется текущее UTC-время.
            flush: Выполнить ли `flush` после обновления.
            refresh: Выполнить ли `refresh` после обновления.
            require_all_parts: Требовать ли загрузку всех частей перед завершением.
    ...
    async def mark_aborted(
        ...
        *,
        ...
        Args:
            upload_session_id: Идентификатор сессии загрузки.
            aborted_at: Дата отмены. Если не передана, используется текущее UTC-время.
            flush: Выполнить ли `flush` после обновления.
            refresh: Выполнить ли `refresh` после обновления.
    ...
    async def mark_expired(
        ...
        *,
        ...
        Args:
            upload_session_id: Идентификатор сессии загрузки.
            expired_at: Дата истечения. Если не передана, используется текущее UTC-время.
            flush: Выполнить ли `flush` после обновления.
            refresh: Выполнить ли `refresh` после обновления.
    ...
    async def mark_expired_sessions(
        ...
        *,
        ...
        По умолчанию обрабатывает сессии, найденные через `find_expired_sessions`.
    ...
    async def register_uploaded_part(
        ...
        *,
        ...
        Метод получает сессию с блокировкой строки `FOR UPDATE`, чтобы снизить риск
        некорректного счётчика при параллельной отметке частей.
    ...
    async def increment_uploaded_parts_count(
        ...
        *,
        ...
        Если сессия находится в статусе `CREATED`, она переводится в `UPLOADING`.

        Args:
            upload_session_id: Идентификатор сессии загрузки.
            increment_by: Количество частей, на которое нужно увеличить счётчик.
            uploaded_bytes_increment: Количество байтов, на которое нужно увеличить прогресс.
            flush: Выполнить ли `flush` после обновления.
            refresh: Выполнить ли `refresh` после обновления.
    ...
    async def set_uploaded_parts_count(
        ...
        *,
        ...
        """Устанавливает точное количество загруженных частей и, при необходимости, байтов.
    ...
    async def recalculate_progress_from_parts(
        ...
        *,
        ...
        Учитываются только части со статусом `UPLOADED`. Метод обновляет количество
        загруженных частей и сумму загруженных байтов.
    ...
    async def find_expired_sessions(
        ...
        *,
        ...
        Raises:
            InvalidQueryError: Если параметры пагинации некорректны
                или передан пустой список статусов.
        """
    ...
    async def find_ready_to_complete_sessions(
        ...
        *,
        ...
        В выборку попадают нетерминальные и неистёкшие сессии, у которых количество
        загруженных частей равно общему количеству частей.
    ...
    async def can_complete_session(
        ...
        *,
        ...
        Returns:
            `True`, если сессия существует и может быть завершена, иначе `False`.
        """
    ...
    async def can_receive_parts(
        ...
        *,
        ...
        Returns:
            `True`, если сессия существует и может принимать части, иначе `False`.
        """
    ...
    def _validate_progress_values(
        ...
        *,
        ...
        Args:
            upload_session: Сессия загрузки, относительно которой выполняется проверка.
