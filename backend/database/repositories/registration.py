...
class RegistrationRequestsRepository(BaseRepository[RegistrationRequest]):
    ...
    Работает с моделью `RegistrationRequest` через асинхронную SQLAlchemy-сессию.
    ...
    async def get_latest_by_username(
        ...
        *,
        ...
        Username предварительно нормализуется. Если заявок несколько, возвращается
        самая новая по дате создания.
    ...
    async def email_has_pending_request(
        ...
        *,
        ...
        Returns:
            `True`, если ожидающая заявка с таким email существует, иначе `False`.
        """
    ...
    async def username_has_pending_request(
        ...
        *,
        ...
        Returns:
            `True`, если ожидающая заявка с таким username существует, иначе `False`.
        """
    ...
    async def create_request(
        ...
        *,
        ...
        или username. Если передан `created_user_id`, может дополнительно проверить,
        что связанный пользователь существует.
        ...
        Args:
            ...
            check_pending_duplicates: Проверять ли дубликаты среди ожидающих заявок.
            check_created_user_exists: Проверять ли существование созданного пользователя.
        ...
        Raises:
            ...
            DuplicateEntityError: Если уже существует ожидающая заявка с таким email
                или username.
            EntityNotFoundError: Если `created_user_id` передан, но пользователь не найден.
        """
    ...
    async def list_reviewed(
        ...
        *,
        ...
        Рассмотренными считаются заявки со статусами `APPROVED`, `REJECTED`
        и `CANCELLED`.

        Args:
            offset: Количество записей, которые нужно пропустить.
            limit: Максимальное количество записей.
            order_by_reviewed_desc: Сортировать ли по дате рассмотрения по убыванию.
    ...
    async def approve_request(
        ...
        *,
        ...
        При необходимости проверяет, что заявка находится в статусе, допускающем
        рассмотрение, и что пользователь, созданный по заявке, существует.
        ...
        Args:
            ...
            reviewed_at: Дата рассмотрения. Если не передана, используется текущее UTC-время.
    ...
    async def cancel_request(
        ...
        *,
        ...
        Args:
            ...
            cancelled_at: Дата отмены. Если не передана, используется текущее UTC-время.
            flush: Выполнить ли `flush` после обновления.
            refresh: Выполнить ли `refresh` после обновления.
            require_pending: Требовать ли, чтобы заявка могла быть рассмотрена.
        ...
        Raises:
            InvalidQueryError: Если включена проверка `require_pending`
                и заявка уже была рассмотрена.
        """
