...
class NodePermissionsRepository(BaseRepository[NodePermission]):
    ...
    async def grant_permission(
        ...
        *,
        ...
        Если запись для пары `node_id + user_id` уже существует, метод переиспользует
        её: обновляет флаги доступа, срок действия, автора выдачи и снимает отзыв
        через `revoked_at=None`.
        ...
        Args:
            ...
            expires_at: Дата и время истечения разрешения. `None` означает бессрочно.
            flush: Выполнить ли `flush` после создания или обновления.
            refresh: Выполнить ли `refresh` после создания или обновления.
    ...
    async def update_permission(
        ...
        *,
        ...
        Для `granted_by`, `expires_at` и `revoked_at` используется sentinel `_UNSET`:
        `_UNSET` означает «не изменять поле», а `None` означает «явно очистить поле».
        ...
        Args:
            ...
            granted_by: Новый автор выдачи доступа, `None` для очистки или `_UNSET` без изменений.
            expires_at: Новый срок действия, `None` для бессрочного доступа или `_UNSET` без изменений.
            revoked_at: Дата отзыва, `None` для восстановления или `_UNSET` без изменений.
    ...
    async def extend_permission(
        ...
        *,
        ...
        Args:
            node_id: Идентификатор узла файловой системы.
            user_id: Идентификатор пользователя.
            expires_at: Новая дата истечения разрешения или `None` для бессрочного доступа.
            flush: Выполнить ли `flush` после обновления.
            refresh: Выполнить ли `refresh` после обновления.
    ...
    async def user_can_share(
        ...
        *,
        ...
        Returns:
            `True`, если пользователь может управлять доступом к узлу, иначе `False`.
        """
    ...
    def _normalize_moment(
        ...
        """Возвращает datetime для проверки срока действия разрешения.
