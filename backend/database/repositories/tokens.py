...
class RefreshTokensRepository(BaseRepository[RefreshToken]):
    ...
    async def create_token(
        ...
        *,
        ...
        проверяет существование пользователя, parent-токена и отсутствие дубликата
        по хэшу.
    ...
    async def revoke_token(
        ...
        *,
        ...
        Args:
            ...
            revoked_at: Дата и время отзыва. Если не передана, используется текущее UTC-время.
            replaced_by_token_id: Идентификатор токена, которым был заменён текущий токен.
    ...
    async def revoke_all_user_tokens(
        ...
        *,
        ...
        Можно исключить один токен из отзыва, например текущую сессию пользователя.
    ...
    async def rotate_token(
        ...
        *,
        ...
        Args:
            ...
            ip_address: IP-адрес клиента.
            user_agent: User-Agent клиента.
            device_name: Название устройства. Если не передано, используется устройство старого токена.
            revoked_at: Дата и время отзыва старого токена.
            revoke_reason: Причина отзыва старого токена.
            flush: Выполнить ли `flush` после операции.
            refresh: Выполнить ли `refresh` для нового токена.
        ...
        Raises:
            InvalidQueryError: Если старый токен не может быть использован или новый хэш некорректен.
            DuplicateEntityError: Если новый хэш токена уже существует.
            RepositoryError: Если произошла ошибка при выполнении операции в базе данных.
        """
    ...
    async def is_token_active(
        ...
        *,
        ...
        Returns:
            `True`, если токен существует и может быть использован, иначе `False`.
        """
    ...
    async def mark_expired_tokens(
        ...
        *,
        ...
        Args:
            expired_before: Момент времени, до которого токены считаются истёкшими.
