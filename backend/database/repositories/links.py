...
class PublicLinksRepository(BaseRepository[PublicLink]):
    ...
    async def create_link(
        ...
        *,
        ...
        Args:
            ...
            max_downloads: Максимальное количество скачиваний или `None` без ограничения.
        ...
        Raises:
            InvalidQueryError: Если token, password hash, описание или лимит скачиваний некорректны.
            EntityNotFoundError: Если узел файловой системы не найден.
            DuplicateEntityError: Если публичная ссылка с таким token уже существует.
        """
    ...
    async def update_link(
        ...
        *,
        ...
        Для nullable-полей используется sentinel `_UNSET`:
        `_UNSET` означает «не изменять поле», а `None` означает «очистить поле».

        Args:
            public_link: ORM-объект публичной ссылки.
            token: Новый token ссылки.
            password_hash: Новый хэш пароля, `None` для очистки или `_UNSET` без изменений.
            permission_type: Новый тип разрешения.
            status: Новый статус ссылки.
            expires_at: Новый срок действия, `None` для бессрочной ссылки или `_UNSET` без изменений.
            max_downloads: Новый лимит скачиваний, `None` без ограничения или `_UNSET` без изменений.
            description: Новое описание, `None` для очистки или `_UNSET` без изменений.
            is_active: Новый признак активности.
            revoked_at: Дата отзыва, `None` для очистки или `_UNSET` без изменений.
            revoked_by: Пользователь, отозвавший ссылку, `None` для очистки или `_UNSET` без изменений.
            revoke_reason: Причина отзыва, `None` для очистки или `_UNSET` без изменений.
        ...
        Raises:
            InvalidQueryError: Если новые значения некорректны
                или `max_downloads` меньше текущего `download_count`.
        """
    ...
    def _normalize_moment(
        ...
        """Возвращает datetime для проверки срока действия ссылки.
