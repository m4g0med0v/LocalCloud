...
class BaseRepository(Generic[ModelT]):
    ...
    async def get_required_one(
        ...
        *conditions: Any,
        ...
        """Возвращает одну обязательную сущность по условиям.
    ...
    async def exists_by_id(
        ...
        Returns:
            True, если сущность с указанным идентификатором существует.
    ...
    async def update(
        ...
        *,
        ...
        Args:
            ...
            allowed_fields: Необязательный whitelist полей, которые разрешено
                обновлять.
    ...
    async def delete_by_id(
        ...
        *,
        ...
        """Физически удаляет сущность по идентификатору.

        Args:
            entity_id: Идентификатор сущности.
            flush: Выполнить ``flush`` после удаления.
            required: Если True, отсутствие сущности считается ошибкой.

        Returns:
            True, если сущность была удалена. False, если сущность не найдена
            и ``required`` равен False.

        Raises:
            EntityNotFoundError: Если сущность не найдена и ``required`` равен
                True.
            ConstraintViolationError: Если нарушено ограничение базы данных.
            RepositoryError: Если операция удаления завершилась ошибкой.
        """
    ...
    async def merge(
        ...
        *,
        ...
        """Выполняет ``merge`` ORM-сущности в текущую сессию.

        Args:
            entity: ORM-сущность для объединения с текущей сессией.
            flush: Выполнить ``flush`` после объединения.

        Returns:
            Объединённая ORM-сущность.

        Raises:
            DuplicateEntityError: Если нарушено ограничение уникальности.
            ConstraintViolationError: Если нарушено ограничение базы данных.
            RepositoryError: Если операция ``merge`` завершилась ошибкой.
        """
    ...
    async def expunge(
        ...
        """Отсоединяет объект от текущей сессии.

        Args:
            entity: ORM-сущность, которую нужно отсоединить от сессии.

        Raises:
            RepositoryError: Если операция отсоединения завершилась ошибкой.
        """
