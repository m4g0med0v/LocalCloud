...
class UserQuotaRepository(BaseRepository[UserQuota]):
    ...
    async def get_by_user_id(
        ...
        *,
        ...
        При необходимости может заблокировать строку квоты через `SELECT FOR UPDATE`,
        что полезно при изменении счётчиков и предотвращении гонок.
    ...
    async def create_quota(
        ...
        *,
        ...
        Raises:
            ...
            ConstraintViolationError: Если текущие значения превышают заданные лимиты.
    ...
    async def update_limits(
        ...
        *,
        ...
        Если параметр `*_limit` передан как `None`, соответствующий лимит снимается.

        Args:
            user_id: Идентификатор пользователя.
            storage_limit_bytes: Новый лимит хранилища в байтах.
            max_file_size_bytes: Новый максимальный размер файла в байтах.
            files_limit: Новый лимит количества файлов, `None` для снятия лимита или `_UNSET` без изменений.
            public_links_limit: Новый лимит публичных ссылок, `None` для снятия лимита или `_UNSET` без изменений.
            active_upload_sessions_limit: Новый лимит активных upload-сессий, `None` для снятия лимита или `_UNSET` без изменений.
        ...
        Raises:
            ...
            ConstraintViolationError: Если текущие значения превышают новые лимиты.
        """
    ...
    async def count_user_active_upload_sessions(
        ...
        Активными считаются upload-сессии в статусах `CREATED` и `UPLOADING`.
    ...
    def _validate_quota_values(
        ...
        *,
        ...
        Валидирует лимиты и текущие счётчики, а также проверяет, что использованные
        значения не превышают соответствующие лимиты.
        ...
        Args:
            ...
            public_links_used: Текущее количество публичных ссылок.
            active_upload_sessions_limit: Лимит активных upload-сессий или `None`.
            active_upload_sessions_used: Текущее количество активных upload-сессий.

        Raises:
            InvalidQueryError: Если одно из числовых значений некорректно.
            ConstraintViolationError: Если использованное значение превышает лимит.
        """
