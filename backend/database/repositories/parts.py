...
class UploadPartsRepository(BaseRepository[UploadPart]):
    ...
    async def create_parts(
        ...
        *,
        ...
        Каждый словарь в `parts` должен содержать `part_number` и `size_bytes`.
        Дополнительно могут быть переданы `etag`, `checksum`, `status`,
    ...
    async def has_missing_etags(
        ...
        Returns:
            `True`, если есть части со статусом `UPLOADED` без ETag, иначе `False`.
        """
