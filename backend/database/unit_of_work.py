...
class UnitOfWork:
    ...
    UnitOfWork инкапсулирует жизненный цикл SQLAlchemy AsyncSession и
    предоставляет единую транзакционную границу для сервисного слоя.

    Поддерживаются два режима работы:

    1. Собственная сессия:
       если `session` не передана, UnitOfWork создаёт новую AsyncSession
       через переданную `session_factory` или через глобальную фабрику
       `get_async_session_factory()`.

    2. Внешняя сессия:
       если `session` передана явно, UnitOfWork использует её, но по
       умолчанию не закрывает при выходе из контекста.

    По умолчанию UnitOfWork не выполняет commit автоматически при успешном
    выходе из контекста. Это сделано намеренно: сервисный слой должен явно
    фиксировать изменения через `await uow.commit()`.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            session:
                Готовая внешняя AsyncSession. Если передана, UnitOfWork не
                создаёт новую сессию.

            session_factory:
                Фабрика SQLAlchemy AsyncSession. Используется только если
                `session` не передана.

            commit_on_exit:
                Если True, UnitOfWork выполнит commit при успешном выходе из
                `async with`, если commit не был выполнен вручную.

            rollback_on_exit_without_commit:
                Если True, UnitOfWork выполнит rollback при успешном выходе из
                контекста, если commit не был выполнен. Это защищает от
                случайного сохранения незавершённых изменений.

            close_session_on_exit:
                Нужно ли закрывать сессию при выходе из контекста.
                Если None, значение определяется автоматически:
                - True для сессии, созданной самим UnitOfWork;
                - False для внешней сессии.
    ...
    async def __aenter__(self) -> Self:
        ...
        Raises:
            UnitOfWorkError:
                Если UnitOfWork уже находится в активном контексте или если
                не удалось создать сессию базы данных.
        """
    ...
    async def __aexit__(
        ...
        Логика завершения:
        - если внутри блока возникло исключение, выполняется rollback;
        - если исключения нет и `commit_on_exit=True`, выполняется commit;
        - если исключения нет, `commit_on_exit=False` и commit не был вызван,
          выполняется rollback при `rollback_on_exit_without_commit=True`;
        - после этого сессия закрывается, если UnitOfWork должен её закрывать.

        Args:
            exc_type:
                Тип исключения, возникшего внутри блока `async with`, или None.

            exc_value:
                Экземпляр исключения, возникшего внутри блока, или None.

            traceback:
                Traceback исключения, возникшего внутри блока, или None.
        ...
        Raises:
            UnitOfWorkError:
                Если при автоматическом commit или закрытии собственной сессии
                произошла ошибка.
        """
    ...
    def session(self) -> AsyncSession:
        ...
        Raises:
            UnitOfWorkError:
                Если сессия не инициализирована или UnitOfWork уже закрыт.
        """
    ...
    def is_active(self) -> bool:
        """
        Проверяет, находится ли UnitOfWork в активном контексте.

        Returns:
            True, если UnitOfWork активен, иначе False.
        """
    ...
    def is_committed(self) -> bool:
        """
        Проверяет, был ли выполнен commit.

        Returns:
            True, если commit был успешно выполнен, иначе False.
        """
    ...
    def is_rolled_back(self) -> bool:
        """
        Проверяет, был ли выполнен rollback.

        Returns:
            True, если rollback был выполнен, иначе False.
        """
    ...
    def owns_session(self) -> bool:
        """
        Проверяет, владеет ли UnitOfWork текущей сессией.

        Returns:
            True, если UnitOfWork сам создал сессию, иначе False.
        """
    ...
    def users(self) -> UsersRepository:
        ...
        Raises:
            UnitOfWorkError:
                Если UnitOfWork используется вне активного контекста.
        """
    ...
    async def commit(self) -> None:
        ...
        - `is_committed` становится True;
        - `is_rolled_back` становится False.
        ...
        Raises:
            ...
            TransactionError:
                Если фиксация транзакции завершилась ошибкой.
        """
    ...
    async def rollback(
        ...
        *,
        ...
        Args:
            suppress_errors:
                Если True, ошибки rollback подавляются. Используется при
                выходе из контекста после исходной ошибки пользовательского
                кода, чтобы не замаскировать её.

        Raises:
            TransactionError:
                Если rollback завершился ошибкой и `suppress_errors=False`.
        """
    ...
    async def refresh(
        ...
        *,
        ...
        Args:
            entity:
                ORM-объект SQLAlchemy, который нужно обновить из базы данных.

            attribute_names:
                Список имён атрибутов для обновления. Если None, обновляется
                вся сущность.
        ...
        Raises:
            ...
            TransactionError:
                Если refresh завершился ошибкой.
        """
    ...
    async def flush_and_refresh(
        ...
        *,
        ...
        сгенерированные базой данных: id, created_at, updated_at и т.д.

        Args:
            entity:
                ORM-объект SQLAlchemy, который нужно отправить в базу данных
                и затем обновить.
        ...
        Raises:
            ...
            TransactionError:
                Если flush или refresh завершились ошибкой.
        """
    ...
    async def close(self) -> None:
        ...
        Raises:
            UnitOfWorkError:
                Если при закрытии собственной сессии произошла ошибка.
        """
    ...
    def _ensure_usable(
        ...
        *,
        ...
        """
        Проверяет, что UnitOfWork находится в активном и пригодном состоянии.

        Args:
            operation:
                Название операции, для которой выполняется проверка. Используется
                в деталях исключения.

        Raises:
            UnitOfWorkError:
                Если UnitOfWork не содержит активной сессии, уже закрыт или
                используется вне активного контекста.
        """
...
class UnitOfWorkFactory:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            session_factory:
                Фабрика SQLAlchemy AsyncSession, которая будет использоваться
                создаваемыми UnitOfWork.

            commit_on_exit:
                Значение по умолчанию для автоматического commit при выходе из
                контекста.

            rollback_on_exit_without_commit:
                Значение по умолчанию для rollback при выходе без явного commit.
        """
    ...
    def __call__(
        ...
        *,
        ...
        Args:
            session:
                Внешняя AsyncSession. Если передана, UnitOfWork будет работать
                с ней и по умолчанию не будет закрывать её при выходе.

            commit_on_exit:
                Переопределяет режим автоматического commit для создаваемого
                UnitOfWork.

            rollback_on_exit_without_commit:
                Переопределяет режим rollback при выходе без явного commit.

            close_session_on_exit:
                Переопределяет правило закрытия сессии при выходе из контекста.
...
def create_unit_of_work_factory(
    *,
    ...
    Args:
        session_factory:
            Фабрика SQLAlchemy AsyncSession. Если None, UnitOfWork будет
            получать глобальную фабрику через `get_async_session_factory()`.

        commit_on_exit:
            Значение по умолчанию для автоматического commit при успешном
            выходе из контекста.
