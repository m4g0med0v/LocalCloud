...
def init_db_client(settings: DatabaseSettings) -> None:
    """
    Инициализировать глобальный асинхронный клиент базы данных.
...
async def close_db_client() -> None:
    """Закрыть клиент базы данных и освободить пул соединений.
...
def get_async_engine() -> AsyncEngine:
    """Вернуть инициализированный AsyncEngine.
...
async def get_db_session() -> AsyncGenerator[AsyncSession, None]:
    """Создать FastAPI dependency для получения AsyncSession.
...
async def ping_database() -> bool:
    """Проверить доступность базы данных запросом SELECT 1.

    Returns:
        True, если база данных доступна.

    Raises:
        DatabaseConnectionError: Если клиент не инициализирован или запрос не
            выполнен.
