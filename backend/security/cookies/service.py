...
def get_cookie_options(settings: Settings | None = None) -> CookieOptions:
    ...
    Returns:
        DTO с параметрами `secure`, `httponly`, `samesite`, `domain` и `path`.
...
def require_refresh_token_from_cookies(
    ...
    *,
    ...
    Raises:
        CookieError: Если refresh token cookie отсутствует, пуста или имя cookie
            некорректно.
    """
...
def get_cookie_value(request: Request, name: str) -> str | None:
    ...
    Returns:
        Значение cookie без пробелов по краям или None, если cookie отсутствует
        либо содержит пустую строку.
