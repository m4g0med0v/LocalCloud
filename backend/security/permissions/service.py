...
from database.models.enums import NodeVisibility, PermissionLevel, SystemRole, UserStatus
...
def check_node_permission(
    *,
    ...
    return PermissionCheckResult(
        ...
        user_id=user_id,
        node_id=node_id,
        permission_level=normalize_permission_level(matched_permission.permission_level),
    )
...
def public_node_allows_action(node: SupportsNode, action: PermissionAction | str) -> bool:
    normalized_action = normalize_permission_action(action)
...
def permission_level_allows_action(
    ...
    required_level = ACTION_REQUIRED_PERMISSION_LEVEL[normalize_permission_action(action)]
...
def permission_level_at_least(
    ...
    return PERMISSION_LEVEL_PRIORITY[actual_level] >= PERMISSION_LEVEL_PRIORITY[
        required_level
    ]
