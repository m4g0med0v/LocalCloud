"""Эндпоинты для управления правами доступа к узлам.

Модуль содержит маршрутизатор FastAPI для просмотра, выдачи, обновления,
отзыва и проверки прав доступа к узлам файловой системы. Также предоставляет
эндпоинт для получения эффективных прав текущего пользователя на конкретный
узел.

Все операции, изменяющие права доступа, выполняются от имени текущего
активного пользователя. Просмотр списка прав на узел дополнительно требует
права на управление доступом к этому узлу.

Attributes:
    router: Маршрутизатор FastAPI с префиксом `/permissions` и тегом
        `permissions`.
"""
