"""Эндпоинты для управления публичными ссылками.

Модуль содержит маршрутизатор FastAPI для создания, просмотра, обновления
и отзыва публичных ссылок на узлы файловой системы. Также предоставляет
публичные эндпоинты для получения информации о ссылке по токену, проверки
доступа и создания ссылки на скачивание через публичный токен.

Административные операции с публичными ссылками требуют текущего активного
пользователя и выполняются от его имени. Публичные маршруты `/public/{token}`,
`/public/{token}/access` и `/public/{token}/download` предназначены для доступа
без пользовательской авторизации, но могут требовать пароль или другие данные,
переданные в теле запроса.

Attributes:
    router: Маршрутизатор FastAPI с префиксом `/public-links` и тегом
        `public-links`.
"""
