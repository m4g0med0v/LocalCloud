...
Модуль содержит маршрутизатор FastAPI для создания, просмотра, отмены
и повторной постановки фоновых задач в очередь. Также предоставляет эндпоинты
для получения результата выполнения задачи и текущего прогресса.

Обычные активные пользователи могут создавать только разрешённые типы задач,
перечисленные в `USER_CREATABLE_TASK_TYPES`, и не могут создавать задачи
от имени другого пользователя. Администраторы могут создавать дополнительные
типы задач, если это разрешено сервисным слоем.

Attributes:
    router: Маршрутизатор FastAPI с префиксом `/tasks` и тегом `tasks`.
    USER_CREATABLE_TASK_TYPES: Набор типов фоновых задач, которые разрешено
        создавать обычным пользователям.
"""
