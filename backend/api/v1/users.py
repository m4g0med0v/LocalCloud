...
Модуль содержит маршрутизатор FastAPI для получения и обновления профиля
текущего пользователя, а также административные эндпоинты для просмотра,
обновления, блокировки, разблокировки, одобрения, отклонения и soft-delete
пользователей.

Операции с профилем выполняются от имени текущего активного пользователя.
Административные операции доступны только текущему администратору.

Attributes:
    router: Маршрутизатор FastAPI с префиксом `/users` и тегом `users`.
"""
