"""Эндпоинты административного аудита.

Модуль содержит маршрутизатор FastAPI для работы с журналом аудита.
Предоставляет API для просмотра событий аудита, получения детальной информации
об отдельном событии, построения агрегированной сводки, экспорта журнала в файл
и просмотра последних событий конкретного пользователя.

Все маршруты модуля предназначены для административного доступа и используют
зависимость `CurrentAdminUserDependency` для проверки текущего пользователя.

Attributes:
    router: Маршрутизатор FastAPI с префиксом `/audit` и тегом `audit`.
"""
