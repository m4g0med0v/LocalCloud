"""Эндпоинты для работы с пользовательскими квотами.

Модуль содержит маршрутизатор FastAPI для получения текущего использования
квоты, административного просмотра квот пользователей, создания или обновления
квоты, проверки возможности расходования ресурсов и запуска пересчёта квоты.

Обычные пользователи могут получать собственное использование квоты и выполнять
проверку доступности расходования ресурсов только для себя. Административные
операции просмотра, создания, обновления и пересчёта квот требуют текущего
пользователя с правами администратора.

Attributes:
    router: Маршрутизатор FastAPI с префиксом `/quotas` и тегом `quotas`.
"""
