"""Сервис управления корзиной LocalCloud.

Модуль содержит бизнес-логику мягкого удаления, восстановления и окончательного
удаления узлов файловой системы. Сервис координирует состояние метаданных в базе
данных, физические объекты в хранилище, счетчики квот, проверки доступа и
события аудита, чтобы операции с файлами, папками, скачиваниями и очисткой
корзины не дублировали эту логику.

Основные операции модуля:
    * Перемещение файла или папки в корзину.
    * Получение одного элемента корзины.
    * Получение списка элементов корзины с фильтрацией и сортировкой.
    * Восстановление узла из корзины.
    * Окончательное удаление выбранных элементов корзины.
    * Полная очистка корзины пользователя.
    * Очистка истекших или старых элементов корзины.
    * Построение плана purge с подсчетом файлов, размера и объектов хранилища.
    * Удаление физических объектов файла и его версий из хранилища.
    * Уменьшение счетчиков квоты после окончательного удаления.
    * Запись событий корзины в аудит.
"""
