"""Сервис управления явными правами доступа к узлам файловой системы.

Модуль содержит бизнес-логику выдачи, обновления, отзыва, чтения и проверки
явных записей NodePermission. Он управляет только явно выданными правами,
а вычисление эффективного доступа делегирует AccessService.

Основные операции модуля:
    * Выдача или замена явного права доступа к узлу.
    * Обновление существующей записи NodePermission.
    * Отзыв права без удаления его истории.
    * Получение одной записи права доступа.
    * Постраничный список прав, выданных на конкретный узел.
    * Постраничный список прав, выданных конкретному пользователю.
    * Проверка эффективного доступа без выброса ошибки при отказе.
    * Получение эффективных флагов доступа пользователя к узлу.
    * Формирование снимков NodePermission для сериализации.
    * Запись событий изменения прав в аудит.
"""
...
def get_permissions_service(
    *,
    ...
    """Создает экземпляр сервиса прав доступа.
