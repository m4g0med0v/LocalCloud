...
Модуль содержит бизнес-логику для liveness, readiness и полного health-check
приложения. Он собирает состояние самого приложения, базы данных и хранилища,
нормализует статусы компонентов и агрегирует их в общий статус сервиса.

Основные операции модуля:
    * Возврат liveness-статуса приложения.
    * Проверка готовности приложения к обработке запросов.
    * Выполнение полного health-check приложения и внешних зависимостей.
    * Проверка состояния базы данных с учетом порога задержки.
    * Проверка состояния хранилища с опциональной read/write-проверкой.
    * Нормализация статусов health-check компонентов.
    * Расчет uptime приложения.
"""
