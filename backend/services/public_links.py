"""Сервис управления публичными ссылками на файлы и папки.

Модуль содержит бизнес-логику создания, чтения, обновления, отзыва и
использования публичных ссылок на узлы файловой системы. Публичные ссылки
могут указывать на файлы или папки, иметь пароль, срок действия, лимит
скачиваний и тип разрешенного доступа.

Основные операции модуля:
    * Создание публичной ссылки на файл или папку.
    * Получение и постраничный список публичных ссылок владельца.
    * Обновление параметров публичной ссылки.
    * Отзыв публичной ссылки.
    * Проверка публичного доступа по токену и паролю.
    * Создание предварительно подписанного URL для публичного скачивания файла.
    * Проверка доступности публичной ссылки для просмотра и скачивания.
    * Формирование снимков PublicLink и связанных узлов для сериализации.
    * Запись событий публичных ссылок и ошибок проверки пароля в аудит.
"""
