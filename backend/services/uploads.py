"""
Сервис управления multipart-загрузками файлов.

Модуль содержит бизнес-логику создания и сопровождения multipart upload-сессий
через предварительно подписанные URL. Сервис координирует проверку доступа к
родительской папке, проверку квот, инициализацию multipart upload в хранилище,
создание URL для частей, фиксацию загруженных частей, завершение загрузки,
создание метаданных файла и обновление счетчиков квоты.

Основные операции модуля:
    * Создание upload-сессии и multipart upload в объектном хранилище.
    * Генерация предварительно подписанных URL для загрузки частей файла.
    * Получение upload-сессий и частей загрузки пользователя.
    * Подтверждение успешной загрузки отдельных частей.
    * Завершение multipart upload и создание File, FileSystemNode и FileVersion.
    * Отмена активной multipart-загрузки.
    * Получение прогресса загрузки для polling-клиентов.
    * Проверка квот пользователя перед началом загрузки.
    * Обновление счетчиков хранилища, файлов и активных upload-сессий.
    * Безопасная отмена orphaned multipart upload при ошибках.
    * Запись событий upload-сессий и загрузки файлов в аудит.
"""
