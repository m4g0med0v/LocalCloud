"""Bootstrap script: create roles + admin user directly in DB."""
import asyncio
import uuid
...
DB_DSN = "postgresql://localcloud:localcloud@localhost:5432/localcloud"
    ...
    try:
        ...
        for code, display in [("admin", "Администратор"), ("user", "Пользователь")]:
            ...
            if not exists:
                await conn.execute(
                    ...
                    uuid.uuid4(), code, display, f"Системная роль {display}",
                )
        ...
        if not admin_id:
            ...
            await conn.execute(
                ...
                admin_id, ADMIN_EMAIL, ADMIN_USERNAME, pw_hash, now,
            )
