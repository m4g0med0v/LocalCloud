...
from storage.types import (
    ...
    StoragePresignedPostPolicyResult,
...
class StoragePresignedUrlManager:
    ...
    async def generate_presigned_put_url(
        ...
        *,
        ...
        Metadata и content-type в этот метод не добавляются намеренно:
        если они должны быть обязательными, их лучше контролировать через
        отдельную upload-логику или POST policy.
    ...
    async def generate_presigned_delete_url(
        ...
        *,
        ...
        Метод оставлен для инфраструктурных сценариев и тестов.
    ...
    async def generate_presigned_upload_part_url(
        ...
        *,
        ...
        URL подписывается как PUT-запрос с query-параметрами ``partNumber``
        и ``uploadId``.
    ...
    async def generate_presigned_post_policy(
        self,
        *,
        bucket: str,
        object_key: str,
        ...
        """Генерирует pre-signed POST policy для загрузки объекта.

        Возвращает URL и набор form-data полей, которые клиент должен
        отправить вместе с файлом.

        Args:
            bucket: Имя bucket.
            object_key: Ключ объекта.
            expires_in_seconds: Срок жизни POST policy в секундах.
            content_type: Обязательный Content-Type загружаемого объекта.
            metadata: Обязательные пользовательские metadata объекта.
            min_size_bytes: Минимальный допустимый размер объекта.
            max_size_bytes: Максимальный допустимый размер объекта.

        Returns:
            DTO с URL, form-data полями и параметрами POST policy.

        Raises:
            StoragePresignedUrlError: Если параметры некорректны или policy не
                удалось сформировать.
            StorageError: Если произошла ошибка хранилища.
        """
        ...
        expires_at = datetime.now(UTC) + timedelta(seconds=normalized_expires)

        self.validate_size_range(
        ...
        policy.add_equals_condition("key", normalized_object_key)

        normalized_content_type = self.normalize_optional_header_value(
            content_type,
            field_name="content_type",
        )

        if normalized_content_type is not None:
            policy.add_equals_condition("Content-Type", normalized_content_type)

        for key, value in normalized_metadata.to_headers().items():
        ...
        if min_size_bytes is not None or max_size_bytes is not None:
            policy.add_content_length_range_condition(
                min_size_bytes or 0,
                max_size_bytes or StorageConstants.S3_POST_POLICY_MAX_OBJECT_SIZE_BYTES,
            )

        try:
            form_data = await self.client.execute(
                self.client.get_raw_client().presigned_post_policy,
                policy,
                operation_name="generate_presigned_post_policy",
            )
        except StorageError as exc:
            raise self._presigned_error(
                exc,
                bucket=normalized_bucket,
                object_key=normalized_object_key,
                method=StoragePresignedUrlMethod.POST,
                expires_in_seconds=normalized_expires,
                operation="generate_presigned_post_policy",
            ) from exc

        if not isinstance(form_data, dict):
            raise StoragePresignedUrlError(
                "MinIO/S3 вернул некорректные данные POST policy.",
                bucket=normalized_bucket,
                object_key=normalized_object_key,
                method=StoragePresignedUrlMethod.POST.value,
                expires_in_seconds=normalized_expires,
                details={
                    "operation": "generate_presigned_post_policy",
                    "value_type": type(form_data).__name__,
                },
            )

        normalized_form_data = {
            str(key): str(value) for key, value in form_data.items()
        }

        normalized_form_data.setdefault("key", normalized_object_key)
        ...
        return StoragePresignedPostPolicyResult(
            url=self._build_post_policy_url(normalized_bucket),
    ...
    def validate_expires_in_seconds(self, expires_in_seconds: int) -> int:
        ...
        Максимум ограничен 7 днями, что соответствует распространённому
        пределу для S3-compatible pre-signed URL.
