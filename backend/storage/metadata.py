...
def normalize_metadata_key(key: str) -> str:
    ...
    Ключ приводится к нижнему регистру, пробелы заменяются на ``_``,
    символ ``-`` сохраняется. Остальные небезопасные символы не допускаются.
...
def normalize_metadata_value(value: Any) -> str | None:
    ...
    Пустые значения возвращаются как None, чтобы затем быть удалёнными
    из итогового набора метаданных.
...
def normalize_metadata(
    ...
    Args:
        metadata: Исходные метаданные в виде mapping, StorageObjectMetadata
            или None.
...
def validate_metadata(
    ...
    Args:
        metadata: Метаданные в виде mapping, StorageObjectMetadata или None.
...
def metadata_to_headers(
    ...
    *,
    ...
    MinIO/S3 принимает пользовательские метаданные как заголовки
    с префиксом ``x-amz-meta-``.
...
def build_file_metadata(
    *,
    ...
    Raises:
        InvalidStorageMetadataError: Если идентификаторы, checksum_algorithm,
            дополнительные метаданные или итоговый набор метаданных некорректны.
    """
...
def merge_metadata(
    *metadata_items: Mapping[str, Any] | StorageObjectMetadata | None,
    ...
    Более поздние наборы переопределяют значения из предыдущих.
...
def _base_metadata(
    *,
    ...
    Raises:
        InvalidStorageMetadataError: Если идентификаторы или checksum_algorithm
            некорректны.
    """
...
def _normalize_checksum_algorithm(
    ...
    Args:
        checksum_algorithm: Алгоритм контрольной суммы в виде enum, строки
            или None.
