...
class StoragePresignedPostPolicyResult:
    """Результат генерации предварительно подписанной POST policy.

    MinIO SDK возвращает form-data поля. Для обычных GET/PUT ссылок
    используется StoragePresignedUrl, а POST policy требует отдельной
    структуры, потому что клиент должен отправить multipart/form-data.
    """
    ...
    def __init__(
        ...
        *,
        ...
        """Создаёт результат генерации предварительно подписанной POST policy.

        Args:
            url: URL для отправки multipart/form-data запроса.
            form_data: Поля формы, которые должен передать клиент.
            bucket: Имя бакета.
            object_key: Ключ объекта.
            expires_in_seconds: Время жизни POST policy в секундах.
            expires_at: Дата и время истечения POST policy.
        """
    ...
    @property
    def expires_in(self) -> int:
        """Возвращает время жизни POST policy.

        Returns:
            Время жизни POST policy в секундах.
        """
    ...
    @property
    def is_success(self) -> bool:
        """Проверяет успешность генерации POST policy.

        Returns:
            True, если POST policy содержит URL и form-data.
        """

        return bool(self.url and self.form_data)

    def to_dict(self) -> dict[str, Any]:
        """Возвращает POST policy в формате, удобном для API-ответа.

        Returns:
            Словарь с URL, form-data, бакетом, ключом объекта и временем
            жизни POST policy.
        """
