...
class StorageClient:
    ...
    ``execute`` для безопасного запуска синхронных операций через
    ``asyncio.to_thread``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Raises:
            StorageConnectionError: Если параметры подключения некорректны
                или не удалось создать MinIO-клиент.
        """
    ...
    def get_raw_client(self) -> Minio:
        ...
        Метод нужен для низкоуровневых менеджеров storage-слоя.
        Использовать его напрямую в бизнес-логике не рекомендуется.
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            coroutine = asyncio.to_thread(operation, *args, **kwargs)
    ...
    async def ping(self, *, timeout_seconds: float | None = 5.0) -> bool:
        ...
        Для ping используется лёгкая операция ``list_buckets``.
    ...
    async def close(self) -> None:
        ...
        Официальный MinIO SDK не требует обязательного закрытия клиента.
        Если переданный ``http_client`` поддерживает ``close`` или ``clear``,
        выполняется попытка освободить ресурсы.

        Raises:
            StorageError: Если не удалось закрыть пользовательский
                HTTP-клиент.
        """
    ...
    def _validate_required_string(value: str, *, field_name: str) -> str:
        ...
        Raises:
            StorageConnectionError: Если значение не является строкой
