...
class StorageMultipartManager:
    ...
    Важно:
        Официальный MinIO Python SDK не предоставляет публичных
        высокоуровневых async-методов multipart upload. Поэтому этот менеджер
        использует низкоуровневые методы клиента MinIO, инкапсулируя их в
        одном месте.
    ...
    def build_completion_parts(
        ...
        Raises:
            StorageMultipartUploadError: Если часть имеет неподдерживаемый
                формат, некорректный номер, пустой ETag или дублирующийся
                номер части.
        """
    ...
    def _read_part_payload(
        *,
        ...
        Raises:
            StorageMultipartUploadError: Если данные имеют некорректный тип
                или размер прочитанной части не совпадает с ожидаемым.
        """
    ...
    def _validate_unique_part_numbers(
        ...
        Raises:
            StorageMultipartUploadError: Если найден дублирующийся номер
                части.
        """
