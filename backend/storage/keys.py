...
def normalize_object_key(object_key: str) -> str:
    ...
    Функция удаляет внешние пробелы, но не пытается автоматически
    исправлять опасные пути: значения с ``..``, обратными слэшами,
    абсолютными путями и повторяющимися слэшами считаются ошибкой.
...
def build_file_object_key(
    *,
    ...
    Физически в MinIO хранится конкретная версия содержимого файла,
    поэтому ключ файла совпадает с ключом версии.
    ...
    Raises:
        InvalidStorageKeyError: Если один из идентификаторов не является UUID
            или итоговый ключ не проходит валидацию.
    """
...
def build_upload_part_object_key(
    *,
    ...
    Raises:
        InvalidStorageKeyError: Если один из идентификаторов не является UUID,
            номер части некорректен или итоговый ключ не проходит валидацию.
    """
...
def build_public_download_object_key(
    *,
    ...
    Этот ключ не обязателен для обычной pre-signed download-логики,
    но может использоваться для подготовленных публичных объектов.
...
def build_trash_object_key(
    *,
    ...
    Обычно soft delete не требует физического перемещения объекта в MinIO,
    но функция полезна для сценариев физического quarantine/trash storage.
...
def build_backup_object_key(
    *,
    ...
    Пользовательское имя файла очищается и используется только как
    техническое имя объекта внутри директории ``backup_id``.
    ...
    Raises:
        InvalidStorageKeyError: Если backup_id не является UUID, prefix
            некорректен или итоговый ключ не проходит валидацию.
    """
...
def extract_extension(filename: str | None) -> str | None:
    ...
    Returns:
        Нормализованное расширение файла без ведущей точки или None,
        если расширение отсутствует.

    Raises:
        InvalidStorageKeyError: Если расширение содержит недопустимые
            символы или превышает максимальную длину.
    """
...
def sanitize_filename_for_metadata(filename: str | None) -> str:
    ...
    Returns:
        Безопасное имя файла для хранения в метаданных или пустую строку,
        если имя не может быть использовано.
    """
...
def get_object_key_parent(object_key: str) -> str | None:
    ...
    Returns:
        Родительский префикс или None, если ключ находится на верхнем уровне.
...
def _normalize_safe_path_part(value: str, *, field_name: str) -> str:
    ...
    Raises:
        InvalidStorageKeyError: Если сегмент не является строкой, пустой,
            является ``.``, ``..`` или содержит разделители пути.
    """
