...
class StorageService:
    ...
    Сервис объединяет bucket/object/multipart/presigned/integrity менеджеры
    и предоставляет методы, удобные для бизнес-сервисов backend.

    Важно:
        Сервис не читает конфигурацию самостоятельно, не обращается к
        PostgreSQL, не открывает транзакции, не проверяет права пользователя
        и не принимает решений о бизнес-доступе к файлам.
    ...
    async def create_upload_post_policy(
        self,
        *,
        bucket: str | None = None,
        expires_in_seconds: int | None = None,
        ...
        """Создаёт pre-signed POST policy для upload через form-data.

        Args:
            bucket: Имя bucket. Если не передан, используется default files
                bucket.
            expires_in_seconds: Срок жизни policy в секундах.
            object_key: Явный ключ объекта.
            user_id: Идентификатор пользователя.
            file_id: Идентификатор файла.
            version_id: Идентификатор версии файла.
            content_type: Обязательный Content-Type.
            metadata: Обязательные metadata.
            min_size_bytes: Минимальный размер объекта.
            max_size_bytes: Максимальный размер объекта.

        Returns:
            DTO с URL и form-data полями.
        """
        ...
        return await self.presigned.generate_presigned_post_policy(
            bucket=resolved_bucket,
            object_key=resolved_object_key,
            expires_in_seconds=resolved_expires,
            content_type=content_type,
