...
def calculate_stream_checksum(
    ...
    *,
    ...
    Если ``reset_position`` равен ``True`` и поток поддерживает ``seek`` /
...
class StorageIntegrityChecker:
    ...
    async def verify_object_checksum(
        ...
        *,
        ...
        Фактический checksum считается по содержимому объекта, а не берётся
        из metadata, потому что metadata не является источником истины.
    ...
    async def verify_object_metadata(
        ...
        *,
        ...
        По умолчанию проверяется, что все ожидаемые metadata присутствуют
        и совпадают. Если ``require_exact_match`` равен ``True``, также
    ...
    async def verify_object_status(
        ...
        *,
        ...
        Так как S3 не хранит прикладной статус, фактический статус берётся
        из ``StorageObjectInfo``. Обычно ``AVAILABLE`` означает, что
