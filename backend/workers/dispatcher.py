...
class WorkerDispatcher:
    """Главный исполнитель фоновых задач worker-процесса."""
    ...
    async def run_once(self) -> WorkerRuntimeStats:
        """Выполняет один цикл обработки задач."""
    ...
    async def fetch_and_lock_tasks(self, limit: int) -> list[BackgroundTask]:
        """Выбирает и блокирует задачи для выполнения текущим worker."""
    ...
    async def execute_task(
        ...
        """Запускает обработчик одной фоновой задачи."""
    ...
    async def mark_stale_running_tasks(self) -> None:
        """Освобождает зависшие RUNNING-задачи с протухшей блокировкой."""
